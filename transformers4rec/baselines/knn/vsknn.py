...
class VMContextKNN:
    ...
    def __init__(
        ...
        self.remind = remind
        self.push_reminders = (
            push_reminders  # give more score to the items that belongs to the current session
        )
        self.add_reminders = (
            add_reminders  # force the last 3 items of the current session to be in the top 20
        )
    ...
    def fit(self, data, items=None):
        ...
        print(
            f"session_item_map: {len(self.session_item_map)} - item_session_map: {len(self.item_session_map)}"
        )
    ...
    def predict_next(
        ...
        items = (
            self.session_items
            if self.last_n_clicks is None
            else self.session_items[-self.last_n_clicks :]
        )
        ...
        known_session_items_for_reminders = [
            item for item in self.session_items if item in series.index
        ]
        ...
        if self.add_reminders:
            session_series = pd.Series(
                index=known_session_items_for_reminders,
                data=series[known_session_items_for_reminders],
            )
        ...
        for session in neighbors:
            ...
            for item in items:
                ...
                new_score = (
                    ...
                    else new_score + (new_score * self.idf[item] * self.idf_weighting)
                )
