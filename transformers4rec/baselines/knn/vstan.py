"""
Based on the implementation from https://github.com/rn5l/session-rec/blob/master/algorithms/knn/vstan.py from the paper
        ...
        if self.session != session_id:  # new session
            ...
            if self.extend:
                ...
                for item in self.session_items:
                    ...
                    self.idf[item] = np.log(self.sessions_count / self.item_freq[item])
        ...
        for session in neighbors:
            ...
            if self.lambda_ipw is not None:
                ...
                for i in range(len(current_session)):
                    if current_session[i] in n_items:
                        ipw_decay = self.session_pos_weight(
                            i + 1, len(current_session), self.lambda_ipw
                        )
            ...
            for item in n_items:
                ...
                if self.lambda_inh is not None:
                    new_score = new_score * self.item_pos_weight(
                        pos_last[item], pos_i_star, self.lambda_inh
                    )
