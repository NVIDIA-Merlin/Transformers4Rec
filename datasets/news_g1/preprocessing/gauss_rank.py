...
class GaussRankScaler(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, epsilon=1e-4, copy=True, n_jobs=None, interp_kind="linear", interp_copy=False
    ):
    ...
    def transform(self, X, copy=None):
        ...
        X = np.array(
            Parallel(n_jobs=self.n_jobs)(delayed(self._transform)(i, x) for i, x in enumerate(X.T))
        ).T
        return X
