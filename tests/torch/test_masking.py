...
def test_task_output_shape(torch_masking_inputs, task):
    ...
    lm.set_masking_targets(torch_masking_inputs["labels"], training=True)
    out = lm(torch_masking_inputs["input_tensor"])
...
def test_mask_only_last_item_for_eval(torch_masking_inputs, task):
    ...
    lm.set_masking_targets(torch_masking_inputs["labels"], training=False)
...
def test_plm_output_shape(torch_masking_inputs):
    ...
    lm.set_masking_targets(torch_masking_inputs["labels"], training=True)
    assert lm.plm_target_mapping is not None
    assert lm.plm_perm_mask is not None
...
def test_replaced_fake_tokens(torch_masking_inputs):
    ...
    lm.set_masking_targets(torch_masking_inputs["labels"], training=True)
    trg_flat = lm.masked_targets.flatten()
