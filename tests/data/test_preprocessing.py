...
def test_remove_consecutive_interactions(synthetic_schema):
    np.random.seed(0)

    schema = synthetic_schema.remove_by_name("item_recency")
...
def test_add_item_first_seen_col_to_df(synthetic_schema):
    schema = synthetic_schema.remove_by_name("item_recency")
    schema += Schema([ColumnSchema.create_continuous("timestamp", tags=[Tag.SESSION])])
