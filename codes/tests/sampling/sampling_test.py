...
class TestCandidateSamplingManager:
    ...
    def test_get_candidate_samples_popularity(self):
        ...
        self._append_interactions_popularity_biased(
            manager, num_sessions=100, session_len=5, max_item_id=100
        )
    ...
    def test_get_candidate_samples_recency(self):
        ...
        self._append_interactions_popularity_biased(
            manager, num_sessions=200, session_len=5, max_item_id=100
        )
    ...
    def test_get_candidate_samples_cooccurrence(self):
        ...
        sampled_ids = []
        for i in range(100):
            sampled_ids.extend(manager.get_candidate_samples(10, item_id=ANCHOR_ITEM_ID))

        assert len(sampled_ids) == 1000

        cooccurrent_item_ids_sorted = sorted(
            list(Counter(sampled_ids).items()), key=lambda x: -x[1]
        )
        # The most co-occurrent item should be the 2
        assert cooccurrent_item_ids_sorted[0][0] == COOCURRENT_ITEM_ID

    def _append_interactions_popularity_biased(
        self, manager, num_sessions, session_len, max_item_id=100
    ):
        INITIAL_TS = 1594133000
        SECS_ELAPSED_BETWEEN_SESSIONS = (
            1200  # 20 minutes (to make 100 sessions last longer than one day)
        )
        ...
        for i in range(1, num_sessions):
            ...
            sampled_item_ids = np.random.choice(
                item_ids, session_len, replace=False, p=item_probs_fixed
            ).tolist()
            ...
            interactions_ts = list(
                [
                    session_start + (t * SECS_BETWEEN_INTERACTIONS)
            ...
            session = {
                ...
                "sess_price_seq": list([i * 100.0 for i in sampled_item_ids]),
            }
            manager.append_session_interactions(session)
