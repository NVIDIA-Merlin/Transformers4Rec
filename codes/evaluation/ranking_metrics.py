...
def precision_at_n(relevant_item_ids, ranked_item_ids, topn, return_mean=True):
    ...
    result = precision_at_n_binary(ranked_items_binary_relevance, topn, return_mean)
    return result
...
def mrr_at_n(relevant_item_ids, ranked_item_ids, topn, return_mean=True):
    ranked_item_ids = ranked_item_ids[:, :topn]
    ranked_items_binary_relevance = is_in_2d_rowwise(ranked_item_ids, in_matrix=relevant_item_ids)
    result = mrr_at_n_binary(ranked_items_binary_relevance, topn, return_mean)
    return result

def mrr_at_n_binary(ranked_items_binary_relevance, topn, return_mean=True):
    ranked_items_binary_relevance = ranked_items_binary_relevance[:, :topn]
...
def map_at_n(relevant_item_ids, ranked_item_ids, topn, return_mean=True):
    ranked_item_ids = ranked_item_ids[:, :topn]
    ranked_items_binary_relevance = is_in_2d_rowwise(ranked_item_ids, in_matrix=relevant_item_ids)
    result = map_at_n_binary(ranked_items_binary_relevance, topn, return_mean)
    return result

def map_at_n_binary(ranked_items_binary_relevance, topn, return_mean=True):
    ranked_items_binary_relevance = ranked_items_binary_relevance[:, :topn]
...
def ndcg_at_n(relevant_item_ids, ranked_item_ids, topn, return_mean=True):
    ranked_items_binary_relevance = is_in_2d_rowwise(ranked_item_ids, in_matrix=relevant_item_ids)
    result = ndcg_at_n_binary(ranked_items_binary_relevance, topn, return_mean)
    return result

def ndcg_at_n_binary(ranked_items_binary_relevance, topn, return_mean=True):
    ...
    perfect_rev_sorting = cp.flip(cp.sort(ranked_items_binary_relevance, axis=1), axis=1)
    idcg = _dcg_at_n(perfect_rev_sorting, topn)
    dcg = _dcg_at_n(ranked_items_binary_relevance, topn)
