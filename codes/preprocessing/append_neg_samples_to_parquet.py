...
def args_parser():
    parser = argparse.ArgumentParser(description='Appends negative samples to existing parquet files.')
    parser.add_argument('--input_parquet_path_pattern', type=str, help='Path to look for the pre-processed parquet files (accepts *)')
    parser.add_argument('--output_parquet_path', type=str, help='Output path with the parquet files appended')
    parser.add_argument('--subsample_first_n_sessions_by_day', type=int, default=0, help='Processes only the first N sessions by date, to reduce output files and speed up processing')
    parser.add_argument('--sampling_strategy', type=str, help='Valid choices: uniform|recency|popularity|cooccurrence')
    parser.add_argument('--num_neg_samples', type=int, help='Number of negative samples for each positive interaction')
    parser.add_argument('--keep_repeated_sampled_items', default=False, action='store_true', help='Indicates if repeated samples items should be kept among negative samples (to keep the sampling probability distribution unchanged)')    
    parser.add_argument('--batch_size', type=int, help='Number of rows for buffered reading from parquet file')
    parser.add_argument('--update_stats_each_n_batches', type=int, help='Updates the stats each N batches')
    parser.add_argument('--save_each_n_batches', type=int, help='Appends new rows to the output parquet file each N batches')
    parser.add_argument('--sliding_windows_last_n_days', type=float, help='Considers as recommendable only items interacted in the last N days')
    parser.add_argument('--recent_temporal_decay_exp_factor', type=float, default=0.002, help='When --sampling_strategy="recency", Configures the exponential decay factor which is used to set the probability of an item to be sampled based on its first interaction (i.e. its age)')
    
    return parserinput_file
...
def get_input_data_config():
    ...
    features_schema = {'sess_pid_seq': FeaturesDataType.LONG,
    ...
    feature_groups = FeatureGroups(
        ...
        item_metadata = ['sess_csid_seq', 'sess_ccid_seq', 'sess_bid_seq', 'sess_price_seq', 'sess_relative_price_to_avg_category_seq', 'sess_product_recency_seq'],
        user_metadata = [], #Not necessary for negative sampling
        event_metadata = [], #Not necessary for negative sampling
        sequential_features=['sess_etime_seq', 'sess_pid_seq', 'sess_ccid_seq', 'sess_ccid_seq', 'sess_bid_seq', 'sess_price_seq', 'sess_relative_price_to_avg_category_seq', 'sess_product_recency_seq'],
    )

    feature_types = FeatureTypes(
        categorical = ['user_idx', 'user_session', 'sess_pid_seq', 'sess_csid_seq', 'sess_ccid_seq', 'sess_bid_seq'], #Not necessary for negative sampling
        numerical = ['sess_etime_seq',  'sess_price_seq', 'sess_relative_price_to_avg_category_seq', 'sess_product_recency_seq'] #Not necessary for negative sampling
    )
...
def generate_neg_samples(session_pids, user_past_pids, n_samples, sampling_manager):
    ...
    for pid in session_pids:
        if pid != 0:
            ...
            neg_item_features_dict = sampling_manager.get_candidate_samples(n_samples, item_id=pid, return_item_features=True, ignore_list=ignore_ids)
...
def main():
    ...
    input_parquet_files = input_parquet_files = sorted(glob.glob(args.input_parquet_path_pattern+'*'))
    ...
    try:
        ...
        for idx_day, input_file in enumerate(input_parquet_files):
            ...
            for batch_id, batch in tqdm(enumerate(split_dataframe_into_chuncks_generator(sessions_df, 
                ...
                if (batch_id % args.update_stats_each_n_batches == args.update_stats_each_n_batches-1) or \
